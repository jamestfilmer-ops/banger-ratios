...
// ─── Album artwork with automatic iTunes fallback ─────────────────────────────
// If the stored artwork_url is broken or wrong, fetches the correct one from
// iTunes using the album's itunes_collection_id. Only fires on actual load
// failure — no extra API calls for albums with working artwork.
function AlbumArtwork({ album }) {
  const [src, setSrc] = useState(
    album.artwork_url?.replace('600x600bb', '100x100bb')
      || album.artwork_url?.replace('600x600', '100x100')
      || album.artwork_url
  )
  const [errored, setErrored] = useState(false)

  async function fetchItunesArtwork() {
    if (!album.itunes_collection_id || errored) return
    try {
      const res  = await fetch(`https://itunes.apple.com/lookup?id=${album.itunes_collection_id}`)
      const data = await res.json()
      const collection = data.results?.find(r => r.wrapperType === 'collection') || data.results?.[0]
      if (collection?.artworkUrl100) {
        setSrc(collection.artworkUrl100)
      } else {
        setErrored(true)
      }
    } catch {
      ...
      {tab === 'rankings' && (
        <>
          ...
          <div style={{
            display: 'flex', gap: 6, flexWrap: 'nowrap',
            overflowX: 'auto', paddingBottom: 4,
            WebkitOverflowStyle: 'none', scrollbarWidth: 'none',
            marginBottom: 20,
          }}>
            {GENRES.map(g => (
